...
function parseCircuit(html: string, sourceUrl: string): F1Circuit | undefined {
  ...
  const facts = [
    ...section.matchAll(
      /<dt[^>]*>([\s\S]*?)<\/dt>\s*<dd[^>]*>([\s\S]*?)<\/dd>(?:\s*<span[^>]*>([\s\S]*?)<\/span>)?/gi,
    ),
  ]
    .map((match) => ({
      label: clean(match[1]),
      value: clean(match[2]),
      detail: clean(match[3] ?? '') || undefined,
    }))
    .filter((fact) => fact.label && fact.value);
  const imageUrl =
    firstMatch(
      section,
      /<img[^>]+src="([^"]+)"[^>]+alt="[^"]*(?:track|Track)[^"]*"/i,
    ) ?? firstMatch(section, /<img[^>]+src="([^"]+)"/i);
  const name = firstMatch(html, /<h1[^>]*>([\s\S]*?)<\/h1>/i) ?? 'Circuit';
  return facts.length > 0
    ? { name: clean(name), imageUrl, facts, sourceUrl }
    : undefined;
}
...
function parseResults(html: string, sourceUrl: string): F1Results | undefined {
  ...
  const table = tables.sort(
    (left, right) =>
      (right.match(/<tr/gi)?.length ?? 0) - (left.match(/<tr/gi)?.length ?? 0),
  )[0];
  if (!table) return undefined;
  const headers = [...table.matchAll(/<th[^>]*>([\s\S]*?)<\/th>/gi)]
    .map((match) => clean(match[1]))
    .filter(Boolean);
  const driverIndex = headers.findIndex(
    (header) => header.toLowerCase() === 'driver',
  );
  const rows = [...table.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)]
    .map((match) => {
      ...
      if (driverIndex >= 0 && values[driverIndex])
        values[driverIndex] = values[driverIndex].replace(/\s+[A-Z]{3}$/, '');
      return { cells: values, driverImageUrl };
    })
    .filter((row) => row.cells.length > 0);
  if (
    rows.some((row) =>
      row.cells.join(' ').toLowerCase().includes('no results available'),
    )
  )
    return undefined;
  ...
  const leaderTime =
    timeIndex >= 0
      ? durationToMilliseconds(rows[0]?.cells[timeIndex] ?? '')
      : undefined;
  if (leaderTime !== undefined) {
    for (const row of rows) {
      ...
      row.cells[timeIndex] = officialTime.startsWith('+')
        ? formatDuration(leaderTime + gap) + ' · ' + officialTime
        : officialTime + ' · LEADER';
    }
  }
  return headers.length > 0 && rows.length > 0
    ? { headers, rows, sourceUrl }
    : undefined;
}
...
export async function getF1OfficialRace(slug: string): Promise<F1OfficialRace> {
  ...
  const [circuitResult, resultsResult] = await Promise.allSettled([
    fetch(sourceUrl).then(async (response) =>
      parseCircuit(await response.text(), sourceUrl),
    ),
    getResults(slug),
  ]);
  const value: F1OfficialRace = {
    circuit:
      circuitResult.status === 'fulfilled' ? circuitResult.value : undefined,
    results:
      resultsResult.status === 'fulfilled' ? resultsResult.value : undefined,
  };
