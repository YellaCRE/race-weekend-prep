...
export default function Home() {
  ...
  return <main>
    ...
    <section id="top" className="hero"><img src="/spa-hero.png" alt="숲을 가로지르는 레이스 서킷" /><div className="hero-shade" /><div className="hero-content"><p className="eyebrow"><span className="live-dot" /> 2026 SEASON · KST</p><h1>RACE WEEKEND,<br /><em>PREP YOUR LAP.</em></h1><p className="hero-copy">F1과 WEC의 순간을 앞두고,<br />당신의 가장 빠른 랩으로 먼저 준비하세요.</p>{nextRace && <a className="next-race" href={'/race/' + raceSlug(nextRace.name)}><span className={'series-pill' + (nextRace.accent === 'gold' ? ' gold' : '')}>{nextRace.series}</span><div><b>{nextRace.name}</b><small>{nextRace.circuit.toUpperCase()} · {nextRace.date}</small></div><ChevronRight size={20} /></a>}</div><div className="hero-stats"><div><strong>07</strong><span>DAYS TO GO</span></div><div><strong>2:18.642</strong><span>SPA PERSONAL BEST</span></div></div></section>
