...
const seedLaps: Lap[] = [
  { id: 1, circuit: 'Spa-Francorchamps', car: 'Ferrari 296 GT3', game: 'Assetto Corsa Competizione', time: '2:18.642', date: '2026.08.27', note: '23°C · Dry · Soft' },
  { id: 2, circuit: 'Spa-Francorchamps', car: 'Ferrari 296 GT3', game: 'Assetto Corsa Competizione', time: '2:19.104', date: '2026.08.20', note: '22°C · Dry · Soft' },
  { id: 3, circuit: 'Suzuka', car: 'Porsche 911 GT3 R', game: 'iRacing', time: '2:01.387', date: '2026.08.14', note: 'Clear · Medium' },
];
...
export default function Home() {
  ...
  useEffect(() => { const saved = window.localStorage.getItem('apex-laps'); if (saved) setLaps(JSON.parse(saved)); }, []);
  useEffect(() => { window.localStorage.setItem('apex-laps', JSON.stringify(laps)); }, [laps]);
  const filteredRaces = races.filter((race) => series === 'ALL' || race.series === series);
  const bestLap = useMemo(() => [...laps.filter((lap) => lap.circuit === 'Spa-Francorchamps' && lap.car === 'Ferrari 296 GT3')].sort((a, b) => parseTime(a.time) - parseTime(b.time))[0], [laps]);
  ...
  return <main>
    ...
    <section id="laps" className="section lap-section"><div className="section-heading"><div><p className="eyebrow dark"><TimerReset size={14} /> SIM RACING LOG</p><h2>나의 랩타임</h2></div><button className="dark-button" onClick={() => setIsFormOpen(true)}><Plus size={16} /> 기록 추가</button></div><div className="lap-layout"><div className="pb-card"><div className="pb-top"><span>PERSONAL BEST</span><Trophy size={20} /></div><h3>{bestLap?.time ?? '—'}</h3><p>SPA-FRANCORCHAMPS · FERRARI 296 GT3</p><div className="pb-footer"><span><Gauge size={14} /> ACC · DRY</span><span className="improvement">▲ 0.462</span></div></div><div className="lap-list">{laps.slice(0, 4).map((lap, index) => <article className="lap-row" key={lap.id}><span className="rank">{String(index + 1).padStart(2, '0')}</span><div><b>{lap.circuit}</b><p>{lap.car} · {lap.game}</p></div><div className="lap-time"><strong>{lap.time}</strong><small>{lap.date}</small></div></article>)}</div></div></section>
    <footer>APEX LAP <span>BUILT FOR THE LAP THAT MATTERS.</span></footer>
    {notice && <div className="toast">{notice}</div>}
    {isFormOpen && <div className="modal-backdrop" role="presentation"><form className="lap-modal" action={saveLap}><button className="close" type="button" onClick={() => setIsFormOpen(false)} aria-label="닫기"><X size={20} /></button><p className="eyebrow dark">NEW LAP</p><h2>랩타임 기록</h2><label>게임<select name="game" defaultValue="Assetto Corsa Competizione"><option>Assetto Corsa Competizione</option><option>iRacing</option><option>Gran Turismo 7</option><option>Le Mans Ultimate</option></select></label><label>서킷<select name="circuit" defaultValue="Spa-Francorchamps"><option>Spa-Francorchamps</option><option>Monza</option><option>Suzuka</option><option>Fuji Speedway</option></select></label><label>차량<input required name="car" defaultValue="Ferrari 296 GT3" /></label><label>랩타임<input required name="time" placeholder="예: 2:18.642" pattern="[0-9]+:[0-9]{2}\\.[0-9]{3}" /></label><label>주행 메모<input name="note" placeholder="예: Dry · Soft · 23°C" /></label><button className="save" type="submit">기록 저장 <ChevronRight size={16} /></button></form></div>}
  </main>;
}
