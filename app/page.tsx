...
const seedRaces: Race[] = [
  { series: 'F1', date: 'SEP 04–06', day: '06', name: 'Italian Grand Prix', circuit: 'Monza', country: 'Italy', time: '22:00', accent: 'red', sourceUrl: '' },
  { series: 'WEC', date: 'SEP 25–27', day: '27', name: '6 Hours of Fuji', circuit: 'Fuji Speedway', country: 'Japan', time: '11:00', accent: 'gold', sourceUrl: '' },
  { series: 'F1', date: 'SEP 11–13', day: '13', name: 'Spanish Grand Prix', circuit: 'Madrid', country: 'Spain', time: '22:00', accent: 'red', sourceUrl: '' },
  { series: 'F1', date: 'SEP 24–26', day: '26', name: 'Azerbaijan Grand Prix', circuit: 'Baku City Circuit', country: 'Azerbaijan', time: '20:00', accent: 'red', sourceUrl: '' },
];
const seedLaps: Lap[] = [
...
export default function Home() {
  ...
  return <main>
    ...
    <section id="calendar" className="section calendar-section"><div className="section-heading"><div><p className="eyebrow dark"><CalendarDays size={14} /> RACE CALENDAR</p><h2>다가오는 레이스</h2><p className="sync-status">{calendarStatus} · 연 1회 자동 가져오기</p></div><div className="filter" aria-label="시리즈 필터">{(['ALL', 'F1', 'WEC'] as const).map((item) => <button key={item} className={series === item ? 'selected' : ''} onClick={() => setSeries(item)}>{item === 'ALL' ? '전체' : item}</button>)}</div></div><div className="race-grid">{filteredRaces.map((race) => <article className="race-card" key={race.series + race.name}><div className="race-date"><span>{race.date}</span><strong>{race.day}</strong></div><div className="race-info"><span className={race.accent === 'red' ? 'badge red' : 'badge gold'}>{race.series}</span><h3>{race.name}</h3><p><MapPin size={14} /> {race.circuit}, {race.country}</p></div><div className="race-time"><Clock3 size={15} /><span>RACE</span><b>{race.time}</b></div></article>)}</div></section>
