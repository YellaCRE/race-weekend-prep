...
export default function Home() {
  ...
  const calendarBanners = useMemo(() => {
    ...
    const segments = filteredRaces.flatMap(({ race, range }) => {
      const first = calendarDays.days.findIndex((day) => dateKey(day) === dateKey(range.start));
      const last = calendarDays.days.findIndex((day) => dateKey(day) === dateKey(range.end));
  ...
  const calendarRowTemplate = calendarBanners.weekLaneCounts.map((count) => `minmax(${112 + count * 29}px, 1fr)`).join(' ');
  ...
  return <main>
    ...
    <section id="calendar" className="section calendar-section"><div className="section-heading"><div><p className="eyebrow dark"><CalendarDays size={14} /> RACE CALENDAR</p><h2>다가오는 레이스</h2><p className="sync-status">{calendarStatus} · KST 기준 · {filteredRaces.length}개 레이스 주말</p></div><div className="filter" aria-label="시리즈 필터">{(['ALL', 'F1', 'WEC'] as const).map((item) => <button key={item} className={series === item ? 'selected' : ''} onClick={() => setSeries(item)}>{item === 'ALL' ? '전체' : item}</button>)}</div></div><div className="calendar-frame"><div className="calendar-range"><b>{calendarDays.month.toLocaleDateString('ko-KR', { year: 'numeric', month: 'long' })}</b><span>향후 30일 내 레이스 주말</span></div><div className="calendar-weekdays">{weekdayLabels.map((day) => <span key={day}>{day}</span>)}</div><div className="month-grid"><div className="calendar-days" style={{ gridTemplateRows: calendarRowTemplate }}>{calendarDays.days.map((day) => { const isToday = dateKey(day) === dateKey(calendarDays.today); const isOutsideMonth = day.getMonth() !== calendarDays.month.getMonth(); return <div className={'calendar-day' + (isToday ? ' today' : '') + (isOutsideMonth ? ' outside-month' : '')} key={dateKey(day)}><time dateTime={dateKey(day)}>{day.getDate()}</time></div>; })}</div><div className="calendar-banners" style={{ gridTemplateRows: calendarRowTemplate }}>{calendarBanners.banners.map(({ race, first, last, lane }) => <a className={'calendar-weekend ' + race.accent} href={'/race/' + raceSlug(race.name)} key={race.series + race.name + first} style={{ gridColumn: String(first % 7 + 1) + ' / ' + String(last % 7 + 2), gridRow: String(Math.floor(first / 7) + 1), marginTop: `${36 + lane * 29}px` }}><span>{race.date}</span><b>{race.name.replace(' Grand Prix', ' GP')}</b></a>)}</div></div></div>{filteredRaces.length === 0 && <p className="calendar-empty">향후 30일 안에 예정된 {series === 'ALL' ? '' : series + ' '}레이스가 없습니다.</p>}</section>
